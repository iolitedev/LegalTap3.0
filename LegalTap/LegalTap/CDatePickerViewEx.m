...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    {
        NSString *strNewMonth = [self titleForRow:self.todayIndexPath.row forComponent:0];
        int i = 0;
        for (i = 0; self.months; i++)
        {
            NSString *strMonth = [self.months objectAtIndex:i];
            if ([strMonth isEqualToString:strNewMonth])
            {
                break;
            }
        }
        self.selectMonth = [NSString stringWithFormat:@"%02d",i+1];
    }
    if (!self.selectYear.length)
    {
        NSString *yearString = [self titleForRow:self.todayIndexPath.section forComponent:1];
        self.selectYear = [yearString substringWithRange:NSMakeRange(2, 2)];
    }
    
    if (component == MONTH)
    {
        NSString *strNewMonth = [self titleForRow:row forComponent:component];
        int i = 0;
        for (i = 0; self.months; i++)
        {
            NSString *strMonth = [self.months objectAtIndex:i];
    ...
    {
        NSString *yearString = [self titleForRow:row forComponent:component];
        self.selectYear = yearString;//[yearString substringWithRange:NSMakeRange(2, 2)];
    }
    
    NSString *strMonth = self.selectMonth;
    NSString *strYear = self.selectYear;
    NSString *selectDate = [NSString stringWithFormat:@"%@/%@",strMonth,strYear];
    
    if ([_dataDelegate respondsToSelector:@selector(CDatePickerViewEx:didSelectDate:)])
    {
        [_dataDelegate CDatePickerViewEx:self didSelectDate:selectDate];
    }
}
